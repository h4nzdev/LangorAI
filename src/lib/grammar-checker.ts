...
const RULES: Rule[] = [
  ...
  // ── Subject-verb agreement ──────────────────────────────────────────────────
  {
    ...
    apply: t => t.replace(/\b(he|she|it)\s+(do not|don't)\b/gi, (_, s) => `${s} doesn't`),
    reason: 'Use "doesn\'t" with he / she / it, not "don\'t".',
  },
  {
    id: 'i-you-we-they-doesnt',
    test: /\b(I|you|we|they)\s+(does not|doesn't)\b/i,
    apply: t => t.replace(/\b(I|you|we|they)\s+(does not|doesn't)\b/gi, (_, s) => `${s} don't`),
  ...
  },
  {
    ...
    apply: t => t.replace(/\b(he|she|it)\s+do\s+(not\s+)?/gi, (_, s, not) => `${s} does ${not ?? ''}`),
  ...
  },
  {
    ...
    reason: 'Use "I was" (past indicative). "I were" is only used in hypothetical / subjunctive contexts.',
  },
  ...
  // ── Modal verbs + to ────────────────────────────────────────────────────────
  {
    ...
    apply: t => t.replace(/\b(can|could|should|would|will|must|shall|may|might)\s+to\s+/gi, (_, m) => `${m} `),
    reason: 'Modal verbs (can, should, must…) are followed directly by the base verb without "to".',
  },
  ...
  // ── Double negatives ────────────────────────────────────────────────────────
  {
    ...
    apply: t => t.replace(/\b(don't|doesn't|didn't|do not|does not|did not)\s+have\s+no\b/gi, (_, neg) => `${neg} have any`),
    reason: 'Avoid double negatives. Use "any" instead of "no" after a negative verb.',
  },
  {
    id: 'cant-do-nothing',
    test: /\b(can't|couldn't|won't|wouldn't|don't|doesn't|didn't|never)\b[^.!?]{0,50}\b(nothing|nobody|nowhere|no one)\b/i,
    apply: t => t.replace(/\bnothing\b/gi, 'anything').replace(/\bnobody\b/gi, 'anybody').replace(/\bno one\b/gi, 'anyone').replace(/\bnowhere\b/gi, 'anywhere'),
    reason: 'Avoid double negatives. Use "anything / anybody / anyone / anywhere" after a negative.',
  },
  ...
  // ── Comparison: then vs than ────────────────────────────────────────────────
  {
    ...
    apply: t => t.replace(/\b(better|worse|more|less|bigger|smaller|faster|slower|harder|easier|older|younger|taller|shorter|higher|lower|richer|smarter|stronger|weaker)\s+then\b/gi, (_, w) => `${w} than`),
  ...
  // ── Confusable fixed expressions ────────────────────────────────────────────
  {
    ...
    reason: '"You\'re welcome" = "you are welcome". "Your" is possessive (your bag, your idea).',
  },
  ...
  // ── Article errors (high-confidence patterns only) ──────────────────────────
  {
    ...
    apply: t => t.replace(/\ba\s+(apple|orange|umbrella|elephant|eagle|egg|ice cream|ice|hour|honor|honest|heir|ant|owl|ocean|island|event|accident|error|exam|excuse|example|exception|experience|idea|oven|opening|officer|operation|offer|advantage)\b/gi, (_, w) => `an ${w}`),
  ...
  },
  {
    ...
    apply: t => t.replace(/\ban\s+(university|unique|useful|user|usual|unit|union|uniform|universe|unicorn|European|euphemism|one\b)/gi, (_, w) => `a ${w}`),
    reason: 'Use "a" (not "an") before words that start with a "yoo" sound, like "university".',
  },
  ...
  },
  {
    ...
    apply: t => t.replace(/\b(did|doing|some|my|the|a lot of|many|several)\s+researches\b/gi, (_, d) => `${d} research`),
    reason: '"Research" as a noun is uncountable — say "research", not "researches".',
  },
  ...
  // ── Redundant conjunctions ───────────────────────────────────────────────────
  {
    ...
    apply: t => t.replace(/\b(although|even though|though)(\b[^.!?]{5,100})\bbut\b/i, (_, conj, middle) => `${conj}${middle}`),
    reason: 'Don\'t use "but" after "although / even though". Pick one conjunction.',
  },
  {
    id: 'because-so',
    test: /\bbecause\b[^.!?]{5,100}\bso\b/i,
    apply: t => t.replace(/(\bbecause\b[^.!?]{5,100})\bso\b/i, (_, pre) => pre),
  ...
  // ── Common preposition errors ────────────────────────────────────────────────
  {
    ...
    apply: t => t.replace(/\blisten\s+(music|songs?|audio|podcast|radio|news)\b/gi, (_, w) => `listen to ${w}`),
  ...
  },
  {
    ...
    reason: 'Say "arrive at" a place (or "arrive in" a city/country), not "arrive to".',
  },
  {
    id: 'married-with',
  ...
  },
  {
    ...
    apply: t => t.replace(/\bgood\s+in\s+(playing|doing|writing|speaking|cooking|drawing|singing|dancing|running|swimming|coding|programming)\b/gi, (_, w) => `good at ${w}`),
